...
app.post(BASE_PATH + "/lottery-sales/:p", (req,res)=>{
    res.status(405).send("Method not allowed");
});
app.post(BASE_PATH + "/lottery-sales/:p/:y", (req,res)=>{
    res.status(405).send("Method not allowed");
});
app.post(BASE_PATH + "/lottery-sales/:p/:y/:t", (req,res)=>{
    res.status(405).send("Method not allowed");
});
app.post(BASE_PATH + "/lottery-sales/:p/:y/:t/:x", (req,res)=>{
    res.status(405).send("Method not allowed");
});
app.post(BASE_PATH + "/lottery-sales/:p/:y/:t/:x/:k", (req,res)=>{
    res.status(405).send("Method not allowed");
});
		...
		if(filteredSales.length==1){
			sales.forEach((c)=>{
				if(c.province==province && c.year == year){
					var index = sales.indexOf(c);
					sales[index].total = data.total;
					sales[index].xmas = data.xmas;
					sales[index].kid = data.kid;
...
app.put(BASE_PATH + "/lottery-sales/:province", (req,res)=>{
