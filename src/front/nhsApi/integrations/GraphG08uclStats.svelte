<script>
    async function loadGraph(){
        ...
        const resDataOtro = await fetch("https://sos1920-08.herokuapp.com/api/v1/ucl_stats");
        ...
        for (var i in YourData) {
            ...
            auxOther.push(YourData.map(function(d) { return d["season"]})[i]);
            auxOther.push(YourData.map(function(d) { return d["last_title"]})[i]);
            total.push(auxOther); 
        }
        console.log(total);
        Highcharts.chart('container', {
            ...
            title: {
                text: 'Integration with G08 api ucl Stats'
            },
            ...
            series: [{
                ...
                tooltip: {
                    pointFormat: 'Urban and season: <b> {point.y}</b><br>' +
                        'Año: <b> {point.z} </b><br>'
                },
                colorByPoint: true
            }]
